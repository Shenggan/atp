...
def aten_transformer_layer_forward(primals_1, primals_2, primals_3, primals_4, primals_5, primals_6,
    ...
    #(TODO) gather getitem
    if SEQ_PARA:
        getitem = gather_tensor(getitem, tensor_dim=1, group=mesh.get_dim_groups()[0])
    # ===== qkv linear ======
    view_1, view, t = aten_linear_forward(primals_3, primals_4, getitem)
    primals_3 = primals_4 = getitem = None
    ...
    # ===== attn linear ======
    view_4 = torch.empty(batch_,
                         seq_,
                         primals_5.size()[0],
                         device=_unsafe_view_3.device,
                         dtype=_unsafe_view_3.dtype)

    _unsafe_view_5 = torch.empty(batch_,
                                 seq_,
                                 num_heads * dim_per_head,
                                 device=_unsafe_view_3.device,
                                 dtype=_unsafe_view_3.dtype)

    def att_linear_func(_reshape_alias, _unsafe_view_3, chunk=1):
        chunk_tensors = [
            torch.chunk(t, chunk) for t in (view_4, _reshape_alias, _unsafe_view_3, _unsafe_view_5)
        ]
        works = []
        for view_4_chunk, _reshape_alias_chunk, _unsafe_view_3_chunk, _unsafe_view_5_chunk in zip(
                *chunk_tensors):
            bmm_1 = torch.ops.aten.bmm.default(_reshape_alias_chunk, _unsafe_view_3_chunk)
            _unsafe_view_4 = torch.ops.aten._unsafe_view.default(
                bmm_1, [batch_ // chunk, num_heads, seq_, dim_per_head])
            transpose_1 = torch.ops.aten.transpose.int(_unsafe_view_4, 1, 2)
            _unsafe_view_5_chunk_2d = _unsafe_view_5_chunk.view_as(transpose_1)
            torch.ops.aten.clone.out(transpose_1,
                                     memory_format=torch.contiguous_format,
                                     out=_unsafe_view_5_chunk_2d)
            _unsafe_view_5_chunk_ = torch.ops.aten._unsafe_view.default(
                _unsafe_view_5_chunk, [batch_ // chunk, seq_, num_heads * dim_per_head])

            view_4_2d = view_4_chunk.view(view_4_chunk.size()[0] * view_4_chunk.size()[1],
                                          view_4_chunk.size()[2])
            aten_linear_forward(primals_5, None, _unsafe_view_5_chunk_, output_buf=view_4_2d)
    ...
    att_linear_func(_reshape_alias, _unsafe_view_3, chunk=CHUNK)

    # scatter view_4
    if SEQ_PARA:
        view_4 = _scatter_tensor(view_4, dim=1, pg=mesh.get_dim_groups()[0])
    ...
    if SEQ_PARA:
        getitem_6 = gather_tensor(getitem_6, tensor_dim=1, group=mesh.get_dim_groups()[0])

    # prepare buffer for feedforward
    view_6 = torch.empty(getitem_6.size()[0],
                         getitem_6.size()[1],
                         primals_10.size()[0],
                         device=getitem_6.device,
                         dtype=getitem_6.dtype)
    gelu = torch.empty_like(view_6)
    view_8 = torch.empty_like(getitem_6)

    def feedforward_func(input_, chunk=1):
        works = []
        chunk_tensors = [torch.chunk(t, chunk) for t in (input_, view_6, gelu, view_8)]
        for input_chunk, view_6_chunk, gelu_chunk, view_8_chunk in zip(*chunk_tensors):
            # ===== ff linear 1 ======
            view_6_2d = view_6_chunk.view(view_6_chunk.size()[0] * view_6_chunk.size()[1],
                                          view_6_chunk.size()[2])
            aten_linear_forward(weight=primals_9,
                                bias=primals_10,
                                input_=input_chunk,
                                output_buf=view_6_2d)

            torch.ops.aten.gelu.out(view_6_chunk, out=gelu_chunk)

            # ===== ff linear 2 ======
            view_8_2d = view_8_chunk.view(view_8_chunk.size()[0] * view_8_chunk.size()[1],
                                          view_8_chunk.size()[2])
            aten_linear_forward(weight=primals_11,
                                bias=None,
                                input_=gelu_chunk,
                                output_buf=view_8_2d)
            works.append(
    ...
    feedforward_func(getitem_6, chunk=CHUNK)
    primals_9 = primals_10 = getitem_6 = None

    if SEQ_PARA:
        view_8 = _scatter_tensor(view_8, dim=1, pg=mesh.get_dim_groups()[0])
...
def aten_transformer_layer_backward(_reshape_alias, view, primals_2, view_7, primals_8,
    ...
    if SEQ_PARA:
        tangents_1_gather = gather_tensor(tangents_1, tensor_dim=1, group=mesh.get_dim_groups()[0])

    # ===== ff linear 2 ======
    _reshape_alias_2, t_7, view_9 = aten_linear_backward(tangents_1_gather, t_3, view_7)
    t_3 = view_7 = None
    ...
    if SEQ_PARA:
        _reshape_alias_4 = _scatter_tensor(_reshape_alias_4, dim=1, pg=mesh.get_dim_groups()[0])
    ...
    if SEQ_PARA:
        add_2 = gather_tensor(add_2, tensor_dim=1, group=mesh.get_dim_groups()[0])
    # ===== attn linear ======
    _reshape_alias_6, t_15, view_11 = aten_linear_backward(add_2, t_1, view_3)
    ...
    if SEQ_PARA:
        _reshape_alias_14 = _scatter_tensor(_reshape_alias_14, dim=1, pg=mesh.get_dim_groups()[0])
...
class AtenTransformerLayer(nn.Module):
    ...
    def __init__(self, dim, dim_per_head, seq_length, mlp_ratio=4) -> None:
        ...
        tp_size = self.mesh.size(0)
        self.qkv_linear_w = nn.Parameter(
            torch.empty((3 * self.dim // tp_size, self.dim), **factory_kwargs))
        self.qkv_linear_b = nn.Parameter(torch.empty(3 * self.dim // tp_size, **factory_kwargs))

        self.att_linear_w = nn.Parameter(
            torch.empty((self.dim, self.dim // tp_size), **factory_kwargs))
        self.att_linear_b = nn.Parameter(torch.empty(self.dim, **factory_kwargs))
        ...
        self.ff1_linear_w = nn.Parameter(
            torch.empty((self.ffn_hidden_size // tp_size, self.dim), **factory_kwargs))
        self.ff1_linear_b = nn.Parameter(
            torch.empty((self.ffn_hidden_size // tp_size), **factory_kwargs))

        self.ff2_linear_w = nn.Parameter(
            torch.empty((self.dim, self.ffn_hidden_size // tp_size), **factory_kwargs))
        self.ff2_linear_b = nn.Parameter(torch.empty((self.dim), **factory_kwargs))
